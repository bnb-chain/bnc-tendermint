...
func newConsensusStateForReplay(config cfg.BaseConfig, csConfig *cfg.ConsensusConfig) *ConsensusState {
	...
	handshaker := NewHandshaker(stateDB, state, blockStore, eventBus, gdoc, true)
	...
	blockExec := sm.NewBlockExecutor(stateDB, log.TestingLogger(), proxyApp.Consensus(), eventBus, mempool, evpool, true)
