...
func NewBlockchainReactor(state sm.State, blockExec *sm.BlockExecutor, store *store.BlockStore,
	fastSync bool, hotSyncReactor, hotSync bool) *BlockchainReactor {
...
// Implements bcRNotifier
func (bcR *BlockchainReactor) switchToConsensusOrHotSync() {
	if bcR.hotSyncReactor && bcR.hotsync {
		bcR.Logger.Info("Time to switch to hot sync reactor!")
		...
		if bcR.hotSyncReactor {
			bcR.Logger.Info("Time to switch hot sync reactor to consensus sync pattern!")
			hotR, ok := bcR.Switch.Reactor("HOT").(hotsyncReactor)
			if ok {
				hotR.SwitchToConsensusSync(bcR.state)
			} else {
				// should only happen during testing
			}
		}
